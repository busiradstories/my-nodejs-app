...
<body>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', async function(e) {
            ...
            // ✅ صح (رابط سيرفري)
            const WEBHOOK_URL = 'https://api.automationdxn.site/webhook/form_site';
    ...
    <script>
        ...
        async function sendMessageToAgent(userMessage) {
            ...
            const WEBHOOK_URL = 'https://api.automationdxn.site/webhook/smartcloser';
    ...
    <script>
        ...
        async function loadSocialLinks() {
            ...
            try {
                ...
                const N8N_DATA_WEBHOOK = 'https://api.automationdxn.site/webhook/get_site_data';

                // 1. إضافة الدومين للرابط ليتمكن السيرفر من معرفة بيانات المالك
                const response = await fetch(`${N8N_DATA_WEBHOOK}?domain=${window.location.hostname}`);
                const data = await response.json();
                
                // 2. استخدام بيانات المالك القادمة من السيرفر (data.owner)
                const profileData = data.owner;

                // const N8N_DATA_WEBHOOK = 'https://api.automationdxn.site/webhook/get_site_data';

                // const contet = await fetch(N8N_DATA_WEBHOOK,{
                //     method: 'GET',
    ...
    <script>
        ...
        async function initSite() {
            try {
                ...
                const response = await fetch(`${N8N_DATA_WEBHOOK}?domain=${currentDomain}`);
